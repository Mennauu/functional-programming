...
2. Alright, we got some data. Now we have to filter out all the data that we don't need. So we map through the (retrieved) data and format it to data that we need and afterwards easily access.
...
3. The last thing we have to do is filter the retrieved books. We only want books which are translated from English to Dutch and books that are English and not translated at all. I created a variable called translatedBooks, mapped through all the results, and then pushed all the books to the variable translatedBooks which have language as Dutch and English as original language.
